...
export const Template = (args, { argTypes }) => ({
  ...
  template: '<unnnic-select-smart v-bind="$props" />',
});
...
<Source
  ...
  dark
  code={`
<unnnic-select-smart :options="exampleArray" />
  `}
/>



The only required property is `options`, which requires an array of objects in the following format:
...
<Canvas>
  <Story
    name='Disabled'
    args={{
      options: [{ value: "2", label: "Option 2" }],
      disabled: true,
    }}>
    {Template.bind({})}
  </Story>
